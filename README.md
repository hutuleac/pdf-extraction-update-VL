...
So every URL the model reads is removed from its text and tables; native text
and OCR keep theirs. The page reports `VLM_URLS_DROPPED` with the count.
...
| `VLM_URLS_DROPPED` | URLs read by the visual model were removed from its text and tables, because the model invents them (11 of 13 on the reference course) — carries the `count`; native and OCR URLs are untouched |
